...
open class ARViewController: UIViewController, ARTrackingManagerDelegate
{
    ...
    open var cameraButtonImage: UIImage? {
        
        didSet {
            
            cameraButton?.setImage(self.cameraButtonImage, for: UIControlState())
            
        }
        
    }
    
    open var floatNoteButtonImage: UIImage? {
        
        didSet {
            
            floatNoteButton?.setImage(self.floatNoteButtonImage, for: UIControlState())
    ...
    fileprivate var floatNoteButton: UIButton?
    ...
    fileprivate var animateLoadText: Int = 8
    ...
    fileprivate func onViewWillAppear()
    {
        ...
        //Camera button
        if self.cameraButton == nil { self.addCameraButton() }
        
        //FloatNote Label
        if self.floatNoteButton == nil { self.addFloatNoteButton() }
    ...
    internal func cameraButtonTap() {
        
        //Take Screenshot and offer to share
        print("camerabutton")
//        
//        let textToShare = "Check out this awesome float note post!"
...
//        self.present(activityVC, animated: true, completion: nil)
    ...
    internal func floatNoteButtonTap()
    {
        print("float tapped")
    }
    ...
    func addCloseButton()
    {
        ...
        if self.closeButtonImage == nil
        {
            ...
            self.closeButtonImage = UIImage(named: "shadow-x")
        ...
        closeButton.frame = CGRect(x: self.view.bounds.size.width - 45, y: 15,width: 25,height: 25)
    ...
    func addCameraButton()
    {
        ...
        if self.cameraButtonImage == nil
        {
            self.cameraButtonImage = UIImage(named: "camera")
        }
        
        // Camerabutton - make it customizable
        let cameraButton: UIButton = UIButton(type: UIButtonType.custom)
        cameraButton.setTitle("Look around to view Float Notes", for: .normal)
        cameraButton.titleLabel?.adjustsFontSizeToFitWidth = true
        cameraButton.frame = CGRect(x: self.view.bounds.size.width / 2, y: self.view.bounds.size.height - 100,width: 300,height: 75)
        cameraButton.center = CGPoint(x: self.view.bounds.size.width / 2, y: self.view.bounds.size.height * 0.5 )
        cameraButton.addTarget(self, action: #selector(ARViewController.cameraButtonTap), for: UIControlEvents.touchUpInside)
    ...
    func animateLoadButton() {
        
        if animateLoadText > 0 {
            
            UIView.animate(withDuration: 1, delay: 0, options: .curveEaseIn, animations: {
                
                self.animateLoadText -= 1
                self.cameraButton!.alpha = 1
                
            }, completion: { finished in
                
                self.animateLoadButton2()
                
            })
        }
    }
    
    func animateLoadButton2() {
    ...
    func addFloatNoteButton() {
        
        self.floatNoteButton?.removeFromSuperview()
        
        if self.floatNoteButtonImage == nil
        {
            self.floatNoteButtonImage = UIImage(named: "name")
        }
        
        // Close button - make it customizable
        let floatNoteButton: UIButton = UIButton(type: UIButtonType.custom)
        floatNoteButton.backgroundColor =  UIColor.clear
//        floatNoteButton.titleLabel?.textColor = UIColor.black
//        floatNoteButton.titleLabel?.text = "No Notes are Visable"
        floatNoteButton.setImage(floatNoteButtonImage, for: UIControlState());
        floatNoteButton.frame = CGRect(x: self.view.bounds.size.width / 2, y: self.view.bounds.size.height / 2,width: 200,height: 70)
        floatNoteButton.center = CGPoint(x: self.view.bounds.size.width / 2, y: self.view.bounds.size.height * 0.05 )
        floatNoteButton.addTarget(self, action: #selector(ARViewController.floatNoteButtonTap), for: UIControlEvents.touchUpInside)
        floatNoteButton.autoresizingMask = [UIViewAutoresizing.flexibleLeftMargin, UIViewAutoresizing.flexibleBottomMargin]
        self.view.addSubview(floatNoteButton)
        self.floatNoteButton = floatNoteButton
    }
    ...
    func addDebugUi()
    {
        ...
        debugLabel.textColor = UIColor.clear
        ...
        debugMapButton.backgroundColor = UIColor.clear
        debugMapButton.setTitleColor(UIColor.clear, for: UIControlState())
